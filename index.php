        ...
        <div class="content">
            ...
            <form method="GET" style="text-align: center;">
                ...
                <select name="category" style="padding: 10px;">
                    ...
                    $db = new SQLite3('rarbg_db.sqlite');

                    // Fetch unique 'cat' values from the 'items' table
                    $query = "SELECT DISTINCT cat FROM items";
                    $result = $db->query($query);

                    // Loop through the results and create options for the dropdown menu
                    while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
                        echo '<option value="' . $row['cat'] . '"';
                        if (isset($_GET['category']) && $_GET['category'] === $row['cat']) {
                            echo ' selected';
                        }
                        echo '>' . $row['cat'] . '</option>';
                    }
            ...
            $searchQuery = "SELECT * FROM items WHERE title LIKE '%" . $searchTerm . "%' ORDER BY cat ASC LIMIT " . $itemsPerPage . " OFFSET " . $offset;
            ...
            $searchCountQuery = "SELECT COUNT(*) as total FROM items WHERE title LIKE '%" . $searchTerm . "%'";
